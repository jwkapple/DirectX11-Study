...
bool DDSClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, const wchar_t** filename)
{
	...
	result = CreateDDSTextureFromFileEx(device, filename[2], 0, D3D11_USAGE_DEFAULT, D3D11_BIND_SHADER_RESOURCE | D3D11_BIND_RENDER_TARGET,
		0, D3D11_RESOURCE_MISC_GENERATE_MIPS, false, (ID3D11Resource**)&mTexture, &mSrvArray[2], nullptr);
	if (FAILED(result))
	{
		return false;
	}

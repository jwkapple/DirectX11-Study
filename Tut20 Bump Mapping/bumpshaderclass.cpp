...
BumpShaderClass::BumpShaderClass()
{
	m_vertexShader = 0;
	m_pixelShader = 0;
	m_layout = 0;
...
BumpShaderClass::BumpShaderClass(const BumpShaderClass&)
{
...
bool BumpShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	bool result;
	
	result = InitializeShader(device, hwnd, (WCHAR*)L"../Tut20 Bump Mapping/vertex_light.hlsl", (WCHAR*)L"../Tut20 Bump Mapping/vertex_light.hlsl");
	if (!result)
	{
		return false;
	}
...
bool BumpShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix,
	XMMATRIX viewMatrix, XMMATRIX projectionMatrix, ID3D11ShaderResourceView** textureArray, XMFLOAT3 lightDirection, XMFLOAT4 diffuseColor)
{
	bool result;
...
bool BumpShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFileName, WCHAR* psFileName)
{
	HRESULT result;
	...
	// 정점 셰이더를 컴파일합니다.
	result = D3DCompileFromFile(vsFileName, NULL, NULL, "BumpVertexShader"
	...
	// 픽셀 셰이더를 컴파일합니다.
	result = D3DCompileFromFile(psFileName, NULL, NULL, "BumpPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS
	...
	// Note that ByteWidth always need to be Bumpple of 16 for createBuffer to work.
...
void BumpShaderClass::ShutdownShader()
{
...
void BumpShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFileName)
{
	char* compileErrors;
...
bool BumpShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix
...
void BumpShaderClass::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
	// 정점 입력 레이아웃을 설정합니다.

#include "multishaderclass.h"

MultiShaderClass::MultiShaderClass()
{
	m_vertexShader = 0;
	m_pixelShader = 0;
	m_layout = 0;
...
MultiShaderClass::MultiShaderClass(const MultiShaderClass&)
{
...
bool MultiShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	bool result;
	
	result = InitializeShader(device, hwnd, (WCHAR*)L"../Tut17 MultiTexturing and Texture Array/vertex_light.hlsl", (WCHAR*)L"../Tut17 MultiTexturing and Texture Array/vertex_light.hlsl");
	if (!result)
	{
		return false;
	}
...
void MultiShaderClass::Shutdown()
{
...
bool MultiShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix,
...
bool MultiShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFileName, WCHAR* psFileName)
{
	HRESULT result;
...
void MultiShaderClass::ShutdownShader()
{
...
void MultiShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFileName)
{
	char* compileErrors;
...
bool MultiShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix
...
void MultiShaderClass::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
	// 정점 입력 레이아웃을 설정합니다.
